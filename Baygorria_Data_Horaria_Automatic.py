...
def Baygorria_Data(Date): #https://apps.ute.com.uy/SgePublico/ConsNivelesPresasHoraria.aspx
...
while Date.strftime("%d/%m/%Y") != stringTDA:
    ...
    if weWrote == 1:
        ...
        with open('Baygorria_Data_Horaria.csv', mode='r') as Baygorria_Data_Horaria:
            Baygorria_Data_Horaria = csv.reader(Baygorria_Data_Horaria, delimiter=',')
            List = list(Baygorria_Data_Horaria)
    ...
    if Date > lastDate:
        ...
        with open('Baygorria_Data_Horaria.csv', mode='a') as Baygorria_Data_Horaria:
            Baygorria_Data_Horaria = csv.writer(Baygorria_Data_Horaria, delimiter=',')
            ...
            for i in range(0,int(L/4)):
                ...
                Baygorria_Data_Horaria.writerow([Date.strftime("%d/%m/%Y"),                                              List15[i*4].string.replace('.','').replace(',','.'),                                              List15[i*4+1].string.replace('.','').replace(',','.'),                                              List15[i*4+2].string.replace('.','').replace(',','.'),                                              List15[i*4+3].string])
            for i in range(0,int(L/4)):
                ...
                Baygorria_Data_Horaria.writerow([Date.strftime("%d/%m/%Y"),                                              List64[i*4].string.replace('.','').replace(',','.'),                                              List64[i*4+1].string.replace('.','').replace(',','.'),                                              List64[i*4+2].string.replace('.','').replace(',','.'),                                      List64[i*4+3].string])
